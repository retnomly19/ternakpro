...
@section('header')
<h2 class="text-2xl font-bold text-white-800 mb-4 text-center">Edit Data Pelanggan</h2>
@endsection
...
<div class="max-w-3xl mx-auto bg-white shadow rounded-lg p-6">
    ...
    <form action="{{ route('pelanggan.update', $pelanggan->id) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" id="id" value="{{ $pelanggan->id }}"
        ...
        <div>
            ...
            <textarea name="alamat" id="alamat" rows="3"
                      class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-pink-500 focus:border-pink-500">{{ old('alamat', $pelanggan->alamat) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="hubungan" id="hubungan"
                    class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-pink-500 focus:border-pink-500">
                <option value="" disabled {{ old('hubungan', $pelanggan->hubungan) ? '' : 'selected' }}>-- Pilih Hubungan --</option>
                <option value="Pihak Ketiga" {{ old('hubungan', $pelanggan->hubungan) == 'Pihak Ketiga' ? 'selected' : '' }}>Pihak Ketiga</option>
                <option value="Pihak Berelasi" {{ old('hubungan', $pelanggan->hubungan) == 'Pihak Berelasi' ? 'selected' : '' }}>Pihak Berelasi</option>
            </select>
        </div>
