...
<div class="max-w-md mx-auto bg-white shadow-lg rounded-2xl p-6">
    ...
    <form action="{{ route('penjualan_ternak.store') }}" method="POST" class="space-y-4 text-sm">
        @csrf
        ...
        <div>
            ...
            <input type="number" name="harga_jual" id="harga_jual" required value="{{ old('harga_jual') }}"
                   class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
        </div>
        ...
        <div>
            ...
            <select name="id_ternak" id="id_ternak" required
                    class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                <option value="" disabled {{ old('id_ternak') ? '' : 'selected' }}>-- Pilih Ternak --</option>
                @foreach($ternakList as $id => $label)
                    <option value="{{ $id }}" {{ old('id_ternak') == $id ? 'selected' : '' }}>
                        {{ $label }}
                    </option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="id_pelanggan" id="id_pelanggan" required
                    class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                <option value="" disabled {{ old('id_pelanggan') ? '' : 'selected' }}>-- Pilih Pelanggan --</option>
                @foreach($pelangganList as $id => $nama)
                    <option value="{{ $id }}" {{ old('id_pelanggan') == $id ? 'selected' : '' }}>
                        {{ $nama }}
                    </option>
                @endforeach
            </select>
