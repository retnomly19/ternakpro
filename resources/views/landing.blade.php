...
<body class="relative bg-white min-h-screen text-slate-900 dark:bg-slate-900 dark:text-slate-100">
    ...
    <header class="relative z-10 max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
        ...
        @if (Route::has('login'))
            <nav class="flex items-center gap-3">
                @auth
                    <a href="{{ url('/dashboard') }}" class="px-4 py-2 rounded bg-amber-500 text-white">Dashboard</a>
                @else
                    <a href="{{ route('login') }}" class="px-4 py-2 rounded text-sky-700 border border-sky-200 bg-white/70 hover:bg-white">Log in</a>
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" class="px-4 py-2 rounded bg-sky-600 text-white hover:bg-sky-700">Register</a>
                    @endif
                @endauth
            </nav>
        @endif
        ...
        <div class="max-w-7xl mx-auto px-6 py-20 lg:py-32">
            <div class="mx-auto max-w-4xl text-center">
                ...
                <div class="mt-10 flex justify-center gap-4">
                    <a href="{{ route('login') }}" class="inline-flex items-center gap-3 px-8 py-4 bg-sky-600 hover:bg-sky-700 text-white rounded-full text-lg shadow-xl transition">Mulai Sekarang
    ...
    <footer class="w-full border-t py-6 mt-12">
        <div class="relative z-10 max-w-6xl mx-auto px-6 text-center text-sm text-slate-600 dark:text-slate-400">
            &copy; {{ date('Y') }} TernakPro. All rights reserved.
        </div>
    </footer>
</body>
</html>
