...
<div class="container mx-auto px-4 py-4 max-w-md">
    ...
    <div class="bg-white shadow-sm rounded-lg p-4">
        <form action="{{ route('kategori_ternak.update', $kategori->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label class="block text-sm font-medium mb-1">Nama Kategori <span class="text-red-600">*</span></label>
                <input type="text" name="nama" value="{{ old('nama', $kategori->nama) }}" required class="w-full border rounded px-3 py-2 text-sm">
            </div>
